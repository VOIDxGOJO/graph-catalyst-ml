...
def featurize_df(df: pd.DataFrame, radius=FP_RADIUS, nBits=FP_NBITS) -> Tuple[np.ndarray, Dict[str, Any]]:
    """
    featurize dataset-
    compute fingerprints (N x nBits)
    create label encoders for agent and solvent (fit on provided df)
    
    return-
    X_fp (numpy array), artifacts dict {agent_le, solvent_le, fp_params, df}
    """
    fps = []
    for smi in df['smiles'].fillna(''):
        fps.append(reaction_to_fp(smi, radius=radius, nBits=nBits))
